...
public class Slider extends GameObject {
    ...
    public void init(final GameObjectListener listener, final Scene scene, final PointF pos, final float offset, final float time, final float r, final float g, final float b, final float scale, int num, final int sound, final int repeats, final float length, final String data, final TimingPoint timing, final String customSound, final String tempSound, final boolean isFirstNote, final double realTime) {
    ...
    public void init(final GameObjectListener listener, final Scene scene, final PointF pos, final float offset, final float time, final float r, final float g, final float b, final float scale, int num, final int sound, final int repeats, final float length, final String data, final TimingPoint timing, final String customSound, final String tempSound, final boolean isFirstNote, final double realTime, SliderPath sliderPath) {
        ...
        for (int i = 1; i <= tickCount; i++) {
            final Sprite tick = SpritePool.getInstance().getCenteredSprite("sliderscorepoint", getPercentPosition((float) (i * tickInterval / (maxTime * GameHelper.getTickRate())), null));
            tick.setScale(scale);
            tick.setAlpha(0);
            ticks.add(tick);
    ...
    private void over() {
        ...
        if (Config.isComplexAnimations() && mWasInRadius) {
            ...
            }, new ScaleModifier(0.2f * GameHelper.getTimeMultiplier(), followCircle.getScaleX(), followCircle.getScaleX() * 0.8f, EaseQuadOut.getInstance()), new AlphaModifier(0.2f * GameHelper.getTimeMultiplier(), followCircle.getAlpha(), 0f, new ModifierListener() {
                @Override
                public void onModifierFinished(IModifier<IEntity> pModifier, IEntity pItem) {
                    SyncTaskManager.getInstance().run(pItem::detachSelf);
                }
    ...
    public void update(final float dt) {
        ...
        if (!startHit) // If we didn't get start hit(click)
        {
            ...
            } else if (replayObjectData != null && Math.abs(replayObjectData.accuracy / 1000f) <= GameHelper.getDifficultyHelper().hitWindowFor50(GameHelper.getDifficulty()) && passedTime + dt / 2 > replayObjectData.accuracy / 1000f) {
                startHit = true;
                Utils.playHitSound(listener, soundId[0], sampleSet[0], addition[0]);
                ticksGot++;
                listener.onSliderHit(id, 30, null, startPosition, false, color, GameObjectListener.SLIDER_START);
        ...
        if (Config.isComplexAnimations()) {
            ...
            if (inRadius && !mWasInRadius) {
                ...
                }, new ScaleModifier(Math.min(remainTime, 0.18f * GameHelper.getTimeMultiplier()), initialScale, scale, EaseQuadOut.getInstance()), new AlphaModifier(Math.min(remainTime, 0.06f * GameHelper.getTimeMultiplier()), followCircle.getAlpha(), 1f)));
                ...
                }, new ScaleModifier(0.1f * GameHelper.getTimeMultiplier(), followCircle.getScaleX(), scale * 2f), new AlphaModifier(0.1f * GameHelper.getTimeMultiplier(), followCircle.getAlpha(), 0f, new ModifierListener() {
                    public void onModifierFinished(IModifier<IEntity> pModifier, IEntity pItem) {
                        if (mIsOver) {
                            SyncTaskManager.getInstance().run(pItem::detachSelf);
                        }
        ...
        while (!ticks.isEmpty() && percentage < 1 - 0.02f / maxTime && tickTime * GameHelper.getTickRate() > tickInterval) {
            ...
            if (followCircle.getAlpha() > 0 && replayObjectData == null || replayObjectData != null && replayObjectData.tickSet.get(tickIndex)) {
                ...
                if (Config.isComplexAnimations() && !mIsAnimating) {
                    followCircle.clearEntityModifiers();
                    followCircle.registerEntityModifier(new ScaleModifier((float) Math.min(tickInterval / GameHelper.getTickRate(), 0.2f) * GameHelper.getTimeMultiplier(), scale * 1.1f, scale, EaseQuadOut.getInstance()));
                }
