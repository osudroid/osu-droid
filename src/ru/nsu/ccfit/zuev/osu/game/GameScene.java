...
public class GameScene implements IUpdateHandler, GameObjectListener, IOnSceneTouchListener {
    ...
    private boolean loadGame(final TrackInfo track, final String rFile) {
        ...
        if (ModMenu.getInstance().isCustomCS()) {
            scale = Config.getRES_HEIGHT() / 480.0f * (54.42f - ModMenu.getInstance().getCustomCS() * 4.48f) * 2f / GameObjectSize.BASE_OBJECT_SIZE + 0.5f * Config.getScaleMultiplier();
        }
    ...
    private void prepareScene() {
        ...
        stat.canFail = !stat.getMod().contains(GameMod.MOD_NOFAIL) && !stat.getMod().contains(GameMod.MOD_RELAX) && !stat.getMod().contains(GameMod.MOD_AUTOPILOT) && !stat.getMod().contains(GameMod.MOD_AUTO);
        ...
        if (stat.getMod().contains(GameMod.MOD_SCOREV2)) {
            ...
            effect.init(fgScene, new PointF(Config.getRES_WIDTH() - effectOffset, 130), scale, new SequenceEntityModifier(ModifierFactory.newScaleModifier(0.25f, 1.2f, 1), ModifierFactory.newDelayModifier(2), new ParallelEntityModifier(ModifierFactory.newFadeOutModifier(0.5f), ModifierFactory.newScaleModifier(0.5f, 1, 1.5f))));
            effectOffset += 25;
            timeOffset += 0.25f;
        }
