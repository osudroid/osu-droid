...
public class OnlineFileOperator {
    ...
    public static void sendFile(String urlstr, String filename, String replayID) {
        try {
            ...
            RequestBody requestBody = new MultipartBody.Builder().setType(MultipartBody.FORM).addFormDataPart("uploadedfile", file.getName(), fileBody).addFormDataPart("hash", checksum).addFormDataPart("replayID", replayID).addFormDataPart("sign", signature).build();
