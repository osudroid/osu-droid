package main.osu.game.cursor.main;
...
import main.osu.game.cursor.trail.CursorTrail;
...
public class CursorEntity extends Entity {
    ...
    public CursorEntity() {
        ...
        if (Config.isUseCursorTrail()) {
            ...
            int amount = (int) Game.activity.getRefreshRate() * 3;

            emitter = new PointParticleEmitter(particleOffsetX, particleOffsetY);
            particles = new CursorTrail(
                    emitter, amount, amount, 1000, cursorSprite.baseSize, trailTex
            );
        }
