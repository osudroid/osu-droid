...
object DroidAimEvaluator {
    private const val WIDE_ANGLE_MULTIPLIER = 1.65
    private const val ACUTE_ANGLE_MULTIPLIER = 1.95
    private const val SLIDER_MULTIPLIER = 1.5
    private const val VELOCITY_CHANGE_MULTIPLIER = 0.85

    private const val SINGLE_SPACING_THRESHOLD = 100.0
    ...
    fun evaluateDifficultyOf(current: DroidDifficultyHitObject, withSliders: Boolean): Double {
        ...
        return snapAimStrainOf(current, withSliders) + flowAimStrainOf(current)
    }

    /**
     * Calculates the snap aim strain of a [DroidDifficultyHitObject].
     */
    private fun snapAimStrainOf(current: DroidDifficultyHitObject, withSliders: Boolean): Double {
    ...
    /**
     * Calculates the flow aim strain of a [DroidDifficultyHitObject].
     */
    private fun flowAimStrainOf(current: DroidDifficultyHitObject): Double {
        var speedBonus = 1.0

        if (current.strainTime < MIN_SPEED_BONUS) {
            speedBonus += 0.75 * ((MIN_SPEED_BONUS - current.strainTime) / 40).pow(2)
        }

        val travelDistance = current.previous(0)?.travelDistance ?: 0.0
        val shortDistancePenalty = (min(
