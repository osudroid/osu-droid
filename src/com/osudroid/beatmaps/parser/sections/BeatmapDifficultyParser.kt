...
object BeatmapDifficultyParser : BeatmapKeyValueSectionParser() {
    override fun parse(beatmap: Beatmap, line: String, scope: CoroutineScope?) = splitProperty(line, scope)?.let {
        when (it.first) {
            "CircleSize" -> {
                val value = parseFloat(it.second).coerceIn(0f, 10f)

                beatmap.difficulty.difficultyCS = value
                beatmap.difficulty.gameplayCS = value
            }

            "OverallDifficulty" -> beatmap.difficulty.od = parseFloat(it.second).coerceIn(0f, 10f)
            "ApproachRate" -> beatmap.difficulty.ar = parseFloat(it.second).coerceIn(0f, 10f)
            "HPDrainRate" -> beatmap.difficulty.hp = parseFloat(it.second).coerceIn(0f, 10f)
