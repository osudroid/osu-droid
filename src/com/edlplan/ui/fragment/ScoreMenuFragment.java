...
public class ScoreMenuFragment extends BaseFragment {
    ...
    protected void onLoadView() {
        findViewById(R.id.exportReplay).setOnClickListener(v -> {
            try {
                var score = DatabaseManager.getScoreTable().getScore(scoreId);
                final File file = new File(
                        new File(Environment.getExternalStorageDirectory(), "osu!droid/export"),
                        String.format(Locale.getDefault(), "%s [%s]-%d.edr",
                                score.getBeatmapFilename().subSequence(score.getBeatmapFilename().indexOf('/') + 1, score.getBeatmapFilename().lastIndexOf('.')),
                                score.getPlayerName(),
                                score.getTime())
                );
                ...
                OsuDroidReplayPack.packTo(file, score);
            ...
            new MessageDialog()
                ...
                .addButton("Yes", dialog -> {
                    ...
                    try {
                        ...
                        if (!ScoreLibrary.deleteScore(scoreId)) {
